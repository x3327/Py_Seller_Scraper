...
AMAZON_SOLD_PHRASES = [
    ...
    # Spanish — amazon.es buybox text variants
    "remitente / vendedor\namazon",
    "vendedor\namazon",
    "vendido por amazon",             # "sold by Amazon" — most common ES form
    "vendido y enviado por amazon",   # "sold and shipped by Amazon"
    "enviado y vendido por amazon",
    "vendido por amazon.es",
    # German
    "verkäufer\namazon",
    "versand durch amazon",           # "fulfilled by Amazon" DE
...
# One category at a time — Railway containers have limited RAM (~512MB).
# Each Chromium context uses ~300MB; running 3 in parallel would OOM the container.
_CATEGORY_CONCURRENCY = 1
...
# One ASIN at a time — RAM constraint on Railway (see category concurrency note above).
_ASIN_CONCURRENCY = 1
...
async def get_seller_id_from_product_page(
    ...
    try:
        ...
        # Wait for product content, then briefly wait for network idle
        # so the JS-rendered buybox (seller link) is present before we check.
        try:
            await page.wait_for_selector(
